...
--MAP MEMBER используетс€ дл€ определени€ методов, которые выполн€ют операции, не измен€ющие состо€ние объекта.
--MEMBER используетс€ дл€ определени€ методов, которые могут измен€ть состо€ние объекта.
...
CREATE TYPE GENRE_TYPE_CONSTRUCTOR AS OBJECT(
    ...
    GENRE_NAME VARCHAR2(50),                                                                  --атрибут
    CONSTRUCTOR FUNCTION GENRE_TYPE_CONSTRUCTOR(P_GENRE_NAME VARCHAR2)RETURN SELF AS RESULT,  --конструктор дл€ создани€ объекта
    MAP MEMBER FUNCTION GET_GENRE_ID RETURN INT,                                              --функци€ получени€ ID
    MEMBER PROCEDURE DISPLAY_GENRE(SELF IN OUT NOCOPY GENRE_TYPE_CONSTRUCTOR)                 --процедура печати
);
--–≈јЋ»«ј÷»я
...
DROP TYPE INTERESTING_FACT_TYPE_INSTANCE;
DROP SEQUENCE SEQ_FACTS;
COMMIT;

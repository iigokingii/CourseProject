...
CREATE OR REPLACE PACKAGE BODY ADMINPROFILEFUNCTIONS AS
    ...
    UNIQUE_LOGIN NUMBER:=0;
    BEGIN
        SELECT COUNT(*) INTO UNIQUE_EMAIL FROM USER_PROFILE 
            WHERE USER_PROFILE.email=UPDATE_USER.EMAIL
                    AND USER_PROFILE.USER_PROFILE_ID<>UPDATE_USER.USER_ID;
        IF UNIQUE_EMAIL = 0 
            THEN
                SELECT COUNT(*) INTO UNIQUE_LOGIN FROM USER_PROFILE 
                    WHERE USER_PROFILE.LOGIN=UPDATE_USER.LOGIN
                        AND USER_PROFILE.USER_PROFILE_ID<>UPDATE_USER.USER_ID;
                IF UNIQUE_LOGIN = 0
                    THEN

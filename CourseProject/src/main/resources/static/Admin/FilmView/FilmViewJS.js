...
async function FillDataToInputs(film){
    ...
    if(film.date_OF_REVIEW!==null || date_OF_REVIEW!==''){
        comments.forEach(comment=>{
            genereteComment(comment.users_REVIEWS_ON_MOVIE_ID,comment.avatar,comment.login,comment.date_OF_REVIEW,comment.user_REVIEW_TEXT);
        })
    }   
}

async function genereteComment(id,avatar,login,reviewDate,commentText){
    ...
    divComment.id = `comment-${id}`;
        ...
        divWrapper.classList.add("commentWrapper");
        ...
        divWrapper.append(divCommentText);
        let divDel = document.createElement('div');
        divDel.classList.add('delBut');
            let buttom = document.createElement('button');
            buttom.innerHTML = 'delete';
            buttom.addEventListener('click',async ()=>{
                console.log(`comment id ${id}`)
                const response = await fetch(`/DeleteComment?commentID=${id}`,{
                    method:"DELETE",
                    headers:{
                        "Accept": "application/json"
                    },
        ...
        divDel.append(buttom);
        divWrapper.append(divDel);
        divComment.append(divWrapper);
